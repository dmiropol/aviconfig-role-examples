...
vars_files=$(ls -v -r vars/vs_vars*.yml 2>/dev/null)
...
for vars_file in $vars_files; do
  cmd="ansible-playbook $playbook -e $password -e @$vars_file -e avi_config_state=$avi_config_state -e avi_role_config_log_mode=$log_mode"
